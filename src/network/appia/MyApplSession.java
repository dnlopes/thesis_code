...
public class MyApplSession extends ApplSession
{
	...
	public MyApplSession(ApplLayer l)
	{
		super(l);
	}

	public void init(int port, SocketAddress multicast, SocketAddress[] gossips, Group group, SocketAddress[]
			viewAddrs)
	{
		this.myPort = port;
	...
	public void initWithSSL(int port, SocketAddress multicast, SocketAddress[] gossips, Group group,
							SocketAddress[] viewAddrs, String keystoreFile, String keystorePass)
	{
	...
	private void printHelp()
	{
		...
		out.println("cast r i m\t Sends message m r times " + "with i tens of a second between them");
		out.println(
				"send d r i m\t Sends message m r times " + "with i tens of a second between them, to member with rank" +
						" d");
	...
	private void sendDebug(StringTokenizer args)
	{
		if(args.countTokens() != 1 && args.countTokens() != 2)
		{
			...
			out.println("debug one of this options: start, stop, now and an optional file name");
			return;
		}
	...
	private void castMessage(StringTokenizer args)
	{
		...
		{
			out.println(
					"The repetitions number and the time  between messages must be " + "an integer. Write \"help\" to " +
		...
		try
		{
			MyApplTimer msgTimer = new MyApplTimer(channel, this, mensagem, resends, period, null);
			msgTimer.go();
		} catch(AppiaEventException ex)
		{
	...
	private void sendMessage(StringTokenizer args)
	{
		...
		try
		{
			MyApplTimer msgTimer = new MyApplTimer(channel, this, mensagem, resends, period, dest);
			msgTimer.go();
		} catch(AppiaEventException ex)
		{
			out.println("There was a problem with message sending");
	...
	public void handle(Event e)
	{
		...
		else if(e instanceof MyApplTimer)
			handleTimer((MyApplTimer) e);
	...
	private void handleNewView(View e)
	{
		...
		out.println((e.ls.am_coord ? "I am" : "I am not") + " the group coordinator");
				/*
	...
	private void handleTimer(MyApplTimer e)
	{
		...
		if(isBlocked)
		{
			...
			{
				out.println("Exception while sending the timer event in a blocked group");
			} catch(AppiaException ex)
			{
				out.println("Exception while sending the timer event in a blocked group");
			}
		...
		try
		{
			...
			if(e.dest != null)
				msgEvent = new ApplSendEvent(channel, this, vs.group, vs.id, e.dest);
			else
			...
			//                }
			//                test.TestOptimized.beginTime2[test.TestOptimized.beginTimes]=test.PrecisionTime
			// .currentTimeMicros();
			//                test.TestOptimized.beginTimes++;
			//              }
			//            }
		...
		{
			out.println("Exception while sending the timer for the next message");
		}
	}
	...
	private void handleChannelInit(ChannelInit e)
	{
		...
		final Thread t = e.getChannel().getThreadFactory().newThread(new MyApplReader(this));
		...
		try
		{
			...
			if(ssl)
			{
				if((keystoreFile != null) && (keystorePass != null))
					rse = new SslRegisterSocketEvent(channel, Direction.DOWN, this, keystoreFile,
							keystorePass.toCharArray());
				else
					rse = new SslRegisterSocketEvent(channel, Direction.DOWN, this);
			} else
			{
				rse = new RegisterSocketEvent(channel, Direction.DOWN, this,
						(myPort < 0) ? RegisterSocketEvent.FIRST_AVAILABLE : myPort);
			}
		...
		} catch(AppiaEventException ex)
		{
			...
			case AppiaEventException.UNWANTEDEVENT:
				System.err.println(
						"The QoS definition doesn't satisfy the " + "application session needs. " +
								"RegisterSocketEvent, received by " + "UdpSimpleSession is not being acepted");
				break;
			default:
				System.err.println("Unexpected exception in " + "Application session");
				break;
		...
		if(multicast != null)
		{
			try
			{
				MulticastInitEvent amie = new MulticastInitEvent(multicast, false, channel, Direction.DOWN, this);
				amie.go();
			} catch(AppiaEventException ex)
			{
	...
	private void sendGroupInit()
	{
		try
		{
			...
			GroupInit gi = new GroupInit(vs, myEndpt, multicast, gossips, channel, Direction.DOWN, this);
			gi.go();
		} catch(AppiaEventException ex)
		{
