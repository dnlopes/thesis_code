...
import network.appia.MyAppLayer;
import network.appia.MyApplSession;
...
public class TestAppia
{
	...
	private static Layer[] qos = {
			...
			new net.sf.appia.protocols.group.bottom.GroupBottomLayer(), new net.sf.appia.protocols.group.heal
			.GossipOutLayer(), new net.sf.appia.protocols.group.suspect.SuspectLayer(), new net.sf.appia.protocols
			.group.intra.IntraLayer(), new net.sf.appia.protocols.group.inter.InterLayer(), new net.sf.appia.protocols
			.group.heal.HealLayer(), new net.sf.appia.protocols.group.stable.StableLayer(), new net.sf.appia.protocols
			.group.leave.LeaveLayer(), new net.sf.appia.protocols.group.sync.VSyncLayer(), new MyAppLayer(),};
	...
	public static void main(String args[])
	{
		...
		for(i = 0; i < args.length; i++)
		{
			...
			} else if(args[i].equals("-gossip"))
			{
				...
				try
				{
					gossips = ParseUtils.parseSocketAddressArray(args[i], InetAddress.getLocalHost(),
							DEFAULT_GOSSIP_PORT);
     ...
     */
		...
		final MyApplSession as = (MyApplSession) qos[qos.length - 1].createSession();
		if(ssl)
    ...
    /* All set. Appia main class will handle the rest */
		//ViewState view = as.getViewSize();
		//System.out.println(view.addresses.length);
		Appia.run();
	}
