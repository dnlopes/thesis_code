...
public class TPCW_Database {
	...
	//public static final int maxConn = 5;
	...
	private static String[] getName(Connection con, int c_id) throws SQLException {
		...
		PreparedStatement get_name = con.prepareStatement("@sql.getName@");
	...
	private static Book getBook(Connection con, int i_id) throws SQLException {
		Book book = null;
		PreparedStatement statement = con.prepareStatement("@sql.getBook@");
	...
	private static Customer getCustomer(Connection con, String UNAME) throws SQLException {
		Customer cust = null;
		PreparedStatement statement = con.prepareStatement("@sql.getCustomer@");
	...
	private static Vector doSubjectSearch(Connection con, String search_key) throws SQLException {
		...
		PreparedStatement statement = con.prepareStatement("@sql.doSubjectSearch@");
	...
	private static Vector doTitleSearch(Connection con, String search_key) throws SQLException {
		Vector vec = new Vector();
		PreparedStatement statement = con.prepareStatement("@sql.doTitleSearch@");
	...
	private static Vector doAuthorSearch(Connection con, String search_key) throws SQLException {
		Vector vec = new Vector();
		PreparedStatement statement = con.prepareStatement("@sql.doAuthorSearch@");
	...
	private static Vector getNewProducts(Connection con, String subject) throws SQLException {
		...
		PreparedStatement statement = con.prepareStatement("@sql.getNewProducts@");
	...
	private static Vector getBestSellers(Connection con, String subject) throws SQLException {
		...
		PreparedStatement statement = con.prepareStatement("@sql.getBestSellers@");
	...
	private static void getRelated(Connection con, int i_id, Vector i_id_vec, Vector i_thumbnail_vec) throws SQLException {
		PreparedStatement statement = con.prepareStatement("@sql.getRelated@");
	...
	private static void adminUpdate(Connection con, int i_id, double cost, String image, String thumbnail) throws SQLException {
		PreparedStatement statement = con.prepareStatement("@sql.adminUpdate@");
		...
		statement.close();
		PreparedStatement related = con.prepareStatement("@sql.adminUpdate.related@");
		...
		{
			// Prepare SQL
			statement = con.prepareStatement("@sql.adminUpdate.related1@");
	...
	private static String GetUserName(Connection con, int C_ID)throws SQLException {
		String u_name = null;
		PreparedStatement get_user_name = con.prepareStatement("@sql.getUserName@");
	...
	private static String GetPassword(Connection con, String C_UNAME)throws SQLException {
		String passwd = null;
		PreparedStatement get_passwd = con.prepareStatement("@sql.getPassword@");
	...
	private static int getRelated1(int I_ID, Connection con) throws SQLException {
		int related1 = -1;
		PreparedStatement statement = con.prepareStatement("@sql.getRelated1@");
		statement.setInt(1, I_ID);
		ResultSet rs = statement.executeQuery();
		rs.next();
	...
	private static Order GetMostRecentOrder(Connection con, String c_uname, Vector order_lines)throws SQLException {
...

		{
			...
			PreparedStatement get_most_recent_order_id = con.prepareStatement("@sql.getMostRecentOrder.id@");
...

		{
			...
			PreparedStatement get_order = con.prepareStatement("@sql.getMostRecentOrder.order@");
...

		{
			...
			PreparedStatement get_order_lines = con.prepareStatement("@sql.getMostRecentOrder.lines@");
	...
	private static int createEmptyCart(Connection con)throws SQLException {
		...
		PreparedStatement insert_cart = con.prepareStatement("@sql.createEmptyCart.insert.v2@");
	...
	private static void addItem(Connection con, int SHOPPING_ID, int I_ID) throws SQLException {
		PreparedStatement find_entry = con.prepareStatement("@sql.addItem@");
		...
		if(rs.next()) {
			...
			currqty+=1;
			PreparedStatement update_qty = con.prepareStatement("@sql.addItem.update@");
			...
			PreparedStatement put_line = con.prepareStatement("@sql.addItem.put@");
	...
	private static void refreshCart(Connection con, int SHOPPING_ID, Vector ids,
		...
		for(i = 0; i < ids.size(); i++){
			...
			if(QTY == 0) { // We need to remove the item from the cart
				PreparedStatement statement = con.prepareStatement("@sql.refreshCart.remove@");
				...
				PreparedStatement statement = con.prepareStatement("@sql.refreshCart.update@");
	...
	private static void addRandomItemToCartIfNecessary(Connection con, int SHOPPING_ID) throws SQLException {
		...
		PreparedStatement get_cart = con.prepareStatement("@sql.addRandomItemToCartIfNecessary@");
	...
	private static void resetCartTime(Connection con, int SHOPPING_ID) throws SQLException {
		PreparedStatement statement = con.prepareStatement("@sql.resetCartTime@");
	...
	private static void refreshSession(Connection con, int C_ID) throws SQLException {
		PreparedStatement updateLogin = con.prepareStatement("@sql.refreshSession@");
	...
	private static Customer createNewCustomer(Connection con, Customer cust) throws SQLException {
		...
		PreparedStatement insert_customer_row = con.prepareStatement("@sql.createNewCustomer@");
		...
		cust.addr_id = enterAddress(con,
				...
				cust.addr_zip,
				cust.co_name);
		PreparedStatement get_max_id = con.prepareStatement("@sql.createNewCustomer.maxId@");
	...
	public static double getCDiscount(Connection con, int c_id) throws SQLException {
		...
		// Prepare SQL
		PreparedStatement statement = con.prepareStatement("@sql.getCDiscount@");
	...
	public static int getCAddrID(Connection con, int c_id) throws SQLException {
		int c_addr_id = 0;
		// Prepare SQL
		PreparedStatement statement = con.prepareStatement("@sql.getCAddrId@");
	...
	public static int getCAddr(Connection con, int c_id) throws SQLException {
		int c_addr_id = 0;
		// Prepare SQL
		PreparedStatement statement = con.prepareStatement("@sql.getCAddr@");
	...
	public static void enterCCXact(Connection con,
		...
		// Prepare SQL
		PreparedStatement statement = con.prepareStatement("@sql.enterCCXact@");
	...
	public static void clearCart(Connection con, int shopping_id) throws SQLException {
		...
		// Prepare SQL
		PreparedStatement statement = con.prepareStatement("@sql.clearCart@");
	...
	public static int enterAddress(Connection con,  // Do we need to do this as part of a transaction?
		...
		// for will be there?
		PreparedStatement get_co_id = con.prepareStatement("@sql.enterAddress.id@");
		...
		//address table
		PreparedStatement match_address = con.prepareStatement("@sql.enterAddress.match@");
		...
		if(!rs.next()){//We didn't match an address in the addr table
			PreparedStatement insert_address_row = con.prepareStatement("@sql.enterAddress.insert@");
	...
	public static int enterOrder(Connection con, int customer_id, Cart cart, int ship_addr_id, String shipping, double c_discount) throws SQLException {
		...
		PreparedStatement insert_row = con.prepareStatement("@sql.enterOrder.insert@");
	...
	public static void addOrderLine(Connection con,
		...
		int success = 0;
		PreparedStatement insert_row = con.prepareStatement("@sql.addOrderLine@");
	...
	public static int getStock(Connection con, int i_id) throws SQLException {
		int stock = 0;
		PreparedStatement get_stock = con.prepareStatement("@sql.getStock@");
	...
	public static void setStock(Connection con, int i_id, int new_stock) throws SQLException {
		PreparedStatement update_row = con.prepareStatement("@sql.setStock@");
	...
	private static void verifyDBConsistency(Connection con) throws SQLException {
		...
		PreparedStatement get_ids = con.prepareStatement("@sql.verifyDBConsistency.custId@");
		...
		get_ids = con.prepareStatement("@sql.verifyDBConsistency.itemId@");
		...
		get_ids = con.prepareStatement("@sql.verifyDBConsistency.addrId@");
		rs = get_ids.executeQuery();
		while(rs.next()){
