...
public class TPCW_Database {
    ...
    public static Connection getNewConnection()
  {
	...
	try 
		...
		try {
		    ...
		    con = DriverManager.getConnection(databaseProp.getDatabaseURL(), databaseProp.getDatabaseUsername(), databaseProp.getDatabasePassword());
                  //con = DriverManager.getConnection("jdbc:mysql://localhost/tpcw", "margueri", "");
                    //con = DriverManager.getConnection("jdbc:cjdbc://sci32::/tpcw", "margueri", "");
		    break;  
    ...
    public static String[] getName(int c_id) 
  {
	...
	try {
	    ...
	    con = getConnection();
	    PreparedStatement statement = con.prepareStatement
		("SELECT * FROM item, author WHERE item.i_a_id = author.a_id AND item.i_subject = ? ORDER BY item.i_title LIMIT 0,50");
	...
	try {
	    ...
	    con = getConnection();
	    PreparedStatement statement = con.prepareStatement
		("SELECT * FROM item, author WHERE item.i_a_id = author.a_id AND substring(soundex(item.i_title),0,4)=substring(soundex(?),0,4) ORDER BY item.i_title LIMIT 0,50");
	...
	try {
	    ...
	    con = getConnection();
	    PreparedStatement statement = con.prepareStatement 
		("SELECT * FROM author, item WHERE substring(soundex(author.a_lname),0,4)=substring(soundex(?),0,4) AND item.i_a_id = author.a_id ORDER BY item.i_title LIMIT 0,50");
	...
	Vector vec = new Vector();  // Vector of Books
        ...
        try 
        {
            ...
            ("SELECT i_id, i_title, a_fname, a_lname , SUM(ol_qty) AS val FROM "+name+", order_line, item, author " +
            "WHERE order_line.ol_o_id = "+name+".o_id AND item.i_id = order_line.ol_i_id AND item.i_subject = ? AND item.i_a_id = author.a_id " +
   ...
   public static void adminUpdate(int i_id, double cost, String image, String thumbnail) 
  {
    Connection con = null;
        try 
        {
            ...
            con = getConnection();
            PreparedStatement related1 = con.prepareStatement
            ("CREATE TEMPORARY TABLE "+name+" TYPE=HEAP SELECT o_id FROM orders ORDER BY o_date DESC LIMIT 10000");
    ...
    private static void resetCartTime(Connection con, int SHOPPING_ID) {
        try {
            PreparedStatement statement = con.prepareStatement("UPDATE shopping_cart SET sc_time = NOW() WHERE sc_id = ?");
            statement.setInt(1, SHOPPING_ID);
            statement.executeUpdate();
            statement.close();
        } catch (java.lang.Exception ex) {
	...
	try 
        {
          ...
          statement1.close();
          // Prepare SQL
          PreparedStatement statement = con.prepareStatement ("INSERT into cc_xacts (cx_o_id, cx_type, cx_num, cx_name, cx_expiry, cx_xact_amt, cx_xact_date, cx_co_id) VALUES (?, ?, ?, ?, ?, ?, CURRENT_DATE(), ?)");
