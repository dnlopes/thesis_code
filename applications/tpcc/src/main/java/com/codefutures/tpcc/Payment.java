					   ...
					   float h_amount_arg	 /* payment amount */)
	{
		try
		{
			...
			try
			{
				...
				if(TRACE)
					logger.trace(
							"UPDATE district SET d_ytd = d_ytd + " + h_amount + " WHERE d_w_id = " + w_id + " AND " +
									"d_id" +
									" = " + d_id);
			...
			if(byname >= 1)
			{
				...
				try
				{
					...
					if(TRACE)
						logger.trace(
								"SELECT c_id FROM customer WHERE c_w_id = " + c_w_id + " AND c_d_id = " + c_d_id + "" +
										" " +
				...
				} catch(SQLException e)
				{
					if(!this.rs.isClosed())
						this.rs.close();
					logger.error(
							"SELECT c_id FROM customer WHERE c_w_id = " + c_w_id + " AND c_d_id = " + c_d_id + " AND" +
									" " +
			...
			if(c_credit != null)
			{
				if(c_credit.contains("BC"))
				{
					...
					c_new_data = String.format("| %d %d %d %d %d $%f %s %s", c_id, c_d_id, c_w_id, d_id, w_id,
							h_amount,
				...
				} else
				{
					...
					try
					{
						...
						if(TRACE)
							logger.trace(
									"UPDATE customer SET c_balance = " + c_balance + " WHERE c_w_id = " + c_w_id + "" +
											" " +
					...
					} catch(SQLException e)
					{
						if(!this.rs.isClosed())
							this.rs.close();
						logger.error(
								"UPDATE customer SET c_balance = " + c_balance + " WHERE c_w_id = " + c_w_id + " AND" +
										" " +
